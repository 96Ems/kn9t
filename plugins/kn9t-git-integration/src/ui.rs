...
local function unified_body(file)
  ...
  for i, row in ipairs(r) do
    if row.hunk then
      ...
      table.insert(items, { spans = {
        { text = mark, fg = "yellow", bold = true },
        { text = string.format("%5s ", num and tostring(num) or ""), fg = "darkgray" },
        { text = prefix .. row.text, fg = line_style(row.kind) },
      }})
...
local function diff_view()
  ...
  local head = string.format("%s  %d/%d  %s  %s",
    ...
    (#V.comments > 0) and (#V.comments .. " comment(s) — C-s to send") or "c to comment")
  ...
  if V.typing ~= nil then
    table.insert(children, {
      type = "text", content = "comment> " .. V.typing,
