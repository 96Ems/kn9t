...
| **effect** | A declared side-effect kind on a tool argument: `Shell`, `FsWrite`, `FsRead`, `Network`. Tools still declare them on the wire, but ADR-0008 removed the server-side mapping — the policy plugin judges the tool name and args (`before_tool_call`), not the declared effects. | ADR-0002, ADR-0008 |
...
| **policy** | A plugin, not a trait. Every installed policy plugin answers `before_tool_call` with `HookVeto` (`Allow`/`Ask`/`Deny`/`Replace`); strictest wins across plugins. The server keeps only the approval *mechanism* (`ApprovalRegistry`, `ApprovalCache`, `Approver`). | §10, ADR-0008 |
...
| **replay fixture** | Raw provider bytes captured via `--record`, replayed by `ReplayProvider`. Enables offline testing of the full loop (truncation ladder, compaction) with no network or spend. | §16, Q22 |
