...
| Field | Type | Description |
|-------|------|-------------|
| `approvals` | string[] | Fingerprints of tool calls approved with scope=always |
| `mode` | `ask_on_mutation` \| `allow_all` \| `deny_all` \| `readonly` | Reporting-only: no verdict derives from it |
| `note` | string |  |
...
---
### `GET /plugin` — plugin inventory — declared name, run state, and the tools each contributes.
...
### `POST /plugin/{name}/start` — respawn a stopped plugin from the command it was originally loaded with, and unblock its tools. Only works on a plugin the server already knows: 404 otherwise (spawning a new command is POST /plugin/load). 409 if already running.
...
### `POST /plugin/{name}/ui_event` — Forward a TUI UI interaction to a plugin. The plugin receives it as a `ui_interaction` event if it subscribed to that kind. 404 when the plugin is unknown.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `data` | object | no | Opaque JSON, forwarded verbatim to the plugin |
| `event` | string | yes | Event name the plugin subscribed to |
...
### `POST /ui-respond` — respond to a pending generic plugin→client interaction (opaque payload, unknown id rejected)
...
| `request` | `id: u64`, `op: `session_read` \| `session_create` \| `session_fork` \| `session_prompt` \| `provider_complete` \| `tool_list` \| `tool_execute` \| `tool_visibility` \| `interaction_request` \| `plugin_list` \| `plugin_start` \| `plugin_stop` \| `plugin_reload` \| `plugin_load` \| `plugin_health` \| `ui_directive` \| `ui_push` \| `ui_register_lua` \| `ui_set_state` \| `ui_clear``, `payload: object` | Plugin → host API request (host_api capability). Ops: session_read (snapshot + transcript), session_create, session_fork, session_prompt (run a turn on the session), provider_complete (one completion on the session's model; usage is recorded as Subagent), tool_list, tool_execute, tool_visibility (a plugin may only change the hidden flag on its own tools), interaction_request (register a pending question; the client answers it with POST /ui-respond), plugin_list, plugin_start, plugin_stop, plugin_reload, plugin_load, plugin_health, ui_directive (alias ui_push), ui_register_lua, ui_set_state, ui_clear. TUI display is plugin-supplied Lua: send the source once with ui_register_lua {source} (defines render(state) returning a widget tree), then push data with ui_set_state {state} (arbitrary JSON) and tear down with ui_clear. The host does not interpret the Lua or the state; the widget vocabulary belongs to the TUI. A registered view is interactive, not render-only: inside its chunk it may call kn9t.on_click(id, fn) to handle clicks on widgets carrying that id, kn9t.on_key(key, fn) to handle keys while the view holds focus, and kn9t.insert_input(text) to append to the user's prompt. Handlers are keyed per plugin, so two plugins may reuse the same id or key without colliding; a handler returning false falls through to the host. Keys only reach a focused view (Esc blurs), so a plugin cannot swallow global keybinds. |
| `result` | `id: u64` |  |
