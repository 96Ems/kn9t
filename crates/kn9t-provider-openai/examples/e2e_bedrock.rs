...
use kn9t_core::{
    CacheMode, Cancel, Content, Message, ModelRef, ModelSpec, MsgId, Price, Provider, Role,
    StopReason, Thinking, ToolSpec,
};
...
fn stream_and_assemble(
    ...
    assemble(chunks, &NullSink).unwrap_or_else(|e| {
        eprintln!("[{label}] FAIL assemble: {e:?}");
        std::process::exit(1);
    })
}
...
fn main() {
    let base_url =
        std::env::var("E2E_BASE_URL").unwrap_or_else(|_| "http://localhost:8080/v1".into());
    let api_key = std::env::var("E2E_API_KEY").ok().filter(|s| !s.is_empty());
    let model_id = std::env::var("E2E_MODEL_ID").unwrap_or_else(|_| "claude-haiku-4-5".into());
    let user_id = std::env::var("E2E_USER_ID").ok().filter(|s| !s.is_empty());
    ...
    let user_q = Message {
        ...
        role: Role::User,
        content: vec![Content::Text {
            text: "What is 7 * 6? Use the calculator tool.".into(),
        }],
        silent: false,
    };
    ...
    assert!(
        matches!(r2a.stop, StopReason::ToolUse),
    ...
    let tool_call = r2a
        ...
        .unwrap_or_else(|| {
            eprintln!("[turn2a] FAIL: no ToolCall in message");
            std::process::exit(1);
        });
    ...
    let expr = serde_json::from_str::<serde_json::Value>(&args_json)
        ...
        .unwrap_or_else(|| {
            eprintln!("[turn2a] FAIL: cannot parse args");
            std::process::exit(1);
        });
    ...
    assert!(
        t2_text.contains("42"),
        "answer must contain 42; got: {t2_text:?}"
    );
    assert!(
        matches!(r2b.stop, StopReason::Stop),
        "turn 2b must stop normally"
    );
