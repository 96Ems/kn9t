use kn9t_core::{CacheMode, Chunk, Effort, ModelRef, ModelSpec, Price, StopReason, Thinking};
use kn9t_provider_core::Quirks;
use kn9t_provider_openai::{
    cache::should_send_cache_fields, decode::DecodeState, encode::build_request, OpenAiConfig,
    OpenAiProvider,
};
use serde_json::json;
...
fn oai_decode_text_stream() {
    ...
    let c1 = state
        .decode(delta1.to_string().as_bytes(), &quirks, &model_ref)
        .unwrap();
    let c2 = state
        .decode(delta2.to_string().as_bytes(), &quirks, &model_ref)
        .unwrap();
...
fn oai_decode_tool_call_stream() {
    ...
    let c1 = state
        .decode(d1.to_string().as_bytes(), &quirks, &model_ref)
        .unwrap();
    let c2 = state
        .decode(d2.to_string().as_bytes(), &quirks, &model_ref)
        .unwrap();
    ...
    let stop = c2
        .iter()
        .find(|c| matches!(c, Chunk::Stop(StopReason::ToolUse)));
    assert!(stop.is_some());
}
...
fn oai_decode_reasoning_stream() {
    ...
    let chunks = state
        .decode(delta.to_string().as_bytes(), &quirks, &model_ref)
        .unwrap();
    let thinking = chunks.iter().find(|c| matches!(c, Chunk::Thinking { .. }));
    assert!(
        thinking.is_some(),
        "expected Thinking chunk from reasoning_content"
    );
}
...
fn oai_toolcall_correlate() {
    ...
    let chunks = state
        .decode(d.to_string().as_bytes(), &quirks, &model_ref)
        .unwrap();
    let tcs: Vec<_> = chunks
        .iter()
...
fn oai_tool_call_encoding() {
    ...
    assert_eq!(
        asst["role"],
        json!("assistant"),
        "assistant role must be set"
    );
    let tool_calls = asst["tool_calls"]
        .as_array()
    ...
    assert!(
        asst.get("type").is_none(),
        "must not emit type:tool_call content part"
    );
    ...
    assert_eq!(
        tr["content"],
        json!("42"),
        "content must be the result text"
    );
}
...
fn oai_cache_automatic_omits() {
    assert!(
        !should_send_cache_fields(&CacheMode::Automatic),
        "Automatic must omit cache fields"
    );
    assert!(
        !should_send_cache_fields(&CacheMode::None),
        "None must omit cache fields"
    );
}
...
fn oai_cache_explicit_places() {
    ...
    assert!(
        should_send_cache_fields(&mode),
        "Explicit must send cache fields"
    );
}
...
fn oai_extra_headers() {
    ...
    assert!(
        default_config.extra_headers.is_empty(),
...
fn nbed_usage_fields() {
    ...
    assert_eq!(
        t1.cache_write, t2.cache_write,
        "cache_write must match regardless of field location"
    );
    assert_eq!(
        t1.cache_read, t2.cache_read,
        "cache_read must match regardless of field location"
    );
...
fn nbed_onem_pair() {
    ...
    // Different ctx.
    assert!(
        full.ctx_window > guard.ctx_window,
        "1M entry must have larger ctx_window"
    );
    // Different prices.
    assert!(
        full.price.input > guard.price.input,
        "1M entry must have higher prices"
    );
    // Different ref.id (200K vs :1m suffix).
    assert_ne!(
        guard.r#ref.id, full.r#ref.id,
        "entries must have different ref.id"
    );
}
...
fn nbed_config_headers() {
    ...
    assert!(
        ...
        "anonymous gateway needs no api_key"
    );
}
...
fn oai_per_model_quirks_override_provider_quirks() {
    ...
    let special = ModelRef {
        provider: "gw".into(),
        id: "special".into(),
    };
    assert_eq!(provider.quirks_for(&special).reasoning, "adaptive");
    assert_eq!(
        provider.quirks_for(&special).max_tokens_field,
...
fn oai_per_model_quirks_reach_the_request_body() {
    ...
    assert!(
        body_base.get("max_tokens").is_some(),
        "provider default field expected"
    );
    assert!(body_base.get("max_completion_tokens").is_none());

    let body_over = build_request(&req, &over, &CacheMode::None, false);
    assert!(
        body_over.get("max_completion_tokens").is_some(),
        "override must reach the body"
    );
...
fn oai_per_model_streaming_override_is_honored() {
    ...
    let streams = ModelRef {
        provider: "gw".into(),
        id: "normal".into(),
    };
    let does_not = ModelRef {
        provider: "gw".into(),
        id: "no-stream".into(),
    };
