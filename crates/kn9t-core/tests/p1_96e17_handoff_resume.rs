...
fn handoff_rejects_unknown_keep() {
    ...
    assert!(
        kn9t_core::validate_handoff(&ev, &known).is_err(),
        "hallucinated keep must be rejected"
    );
}
...
fn resume_from_handoff_reconstructs_context() {
    ...
    let summarize = vec![HandoffSummary {
        id: CallId("sum-1".into()),
        summary: "summarized: did X".into(),
    }];
    ...
    let mut summary_content = vec![Content::Text {
    ...
    for s in &summarize {
        summary_content.push(Content::Text {
            text: format!("{}: {}", s.id.0, s.summary),
        });
    }
    ...
    assert!(
        serialized.contains("LARGE OUTPUT KEEP VERBATIM"),
        "keep must be verbatim in summary"
    );
    assert!(
        !serialized.contains("small output to be summarized"),
        "summarize must NOT contain full original — only summary"
    );
    assert!(
        serialized.contains("summarized: did X"),
        "summarize note must be present"
    );
    // Assert against the plan itself, not a hardcoded literal: every dropped id
    // and its output must be gone from the summary.
    for id in &drop_ids {
        assert!(
            !serialized.contains(&id.0),
            "dropped id {} must be absent",
            id.0
        );
        if let Some((_, output)) = orig.iter().find(|(oid, _)| oid == id) {
            assert!(
                !serialized.contains(output),
                "dropped output must be absent"
            );
        }
    }
...
fn summary_prompt_must_not_contain_full_tool_output() {
    ...
    let full_messages = vec![Message {
        ...
        content: vec![Content::ToolResult {
            id: CallId("t1".into()),
            content: vec![Content::Text {
                text: "FULL SECRET TOOL OUTPUT 0123456789".to_string(),
            }],
            is_error: false,
        }],
        silent: false,
    ...
    assert!(
        !new_prompt.contains("FULL SECRET TOOL OUTPUT"),
        "new prompt must not contain full output"
    );
    ...
    let old_prompt = format!(
        "Decisions:\n{}\n\nSpan:\n{}",
        decisions,
        serde_json::to_string(&full_messages).unwrap()
    );
    assert!(
        old_prompt.contains("FULL SECRET TOOL OUTPUT"),
        "old prompt leaked full output — this is the bug we fixed"
    );
}
