...
impl ConfigSource {
    ...
    fn resolve(dir: &std::path::Path, file: &std::path::Path) -> Option<Self> {
    ...
    fn lua_files_in(dir: &std::path::Path) -> impl Iterator<Item = PathBuf> {
...
impl LuaRuntime {
    ...
    fn load_source(&self, source: &str, name: &str) -> bool {
        ...
        *self.ui_cache.write().unwrap() = None;
        result
    }
    ...
    pub fn reload_all(&self) -> bool {
        let source = self.inner.read().unwrap().config_source.clone();
    ...
    pub fn clear_error(&self) {
        self.inner.write().unwrap().last_error = None;
    }

    /// Check if a Lua config was ever successfully loaded.
    pub fn is_loaded(&self) -> bool {
        self.inner.read().unwrap().ever_loaded
    }
    ...
    pub fn build_ui_outcome(&self, width: u16, height: u16) -> widgets::UiOutcome {
        ...
        {
            let cache = self.ui_cache.read().unwrap();
        ...
        match widgets::parse_widget(&state.lua, &table) {
            Ok(w) => {
                *self.ui_cache.write().unwrap() = Some((fp, w.clone()));
                UiOutcome::Ok(w)
            }
            Err(e) => {
    ...
    pub fn update_state(&self, snap: &state::StateSnapshot) {
        let lua_state = self.inner.read().unwrap();
...
mod tests {
    ...
    fn load_dir_runs_files_in_alphabetical_order() {
        ...
        let order: String = rt.inner.read().unwrap().lua.globals().get("ORDER").unwrap();
        assert_eq!(order, "abc");
    ...
    fn load_dir_stops_at_first_broken_file() {
        ...
        let state = rt.inner.read().unwrap();
        assert_eq!(state.lua.globals().get::<i64>("X").unwrap(), 1);
        assert!(
            state
                .lua
                .globals()
    ...
    fn test_sandbox_no_io() {
        ...
        let os: Table = state.lua.globals().get("os").unwrap();
        let exec: Value = os.get("execute").unwrap();
    ...
    fn test_sandbox_safe_functions_available() {
        ...
        let pairs: Value = state.lua.globals().get("pairs").unwrap();
        assert!(matches!(pairs, Value::Function(_)));

        let tostring: Value = state.lua.globals().get("tostring").unwrap();
        assert!(matches!(tostring, Value::Function(_)));
    }
    ...
    fn test_error_does_not_panic() {
        ...
        // Load invalid Lua - should not panic, should set error
        let mut state = rt.inner.write().unwrap();
        state.ever_loaded = true; // Pretend we loaded

        let result = state.lua.load("this is not valid lua {{{{").exec();
        assert!(result.is_err());
        // The runtime should still be usable
        let valid = state.lua.load("x = 1").exec();
        assert!(valid.is_ok());
    }
    ...
    fn call_count(rt: &LuaRuntime) -> i64 {
        let state = rt.inner.read().unwrap();
        state.lua.globals().get::<i64>("CALLS").unwrap_or(-1)
    }
    ...
    fn changed_message_count_invalidates_the_cache() {
        ...
        rt.update_state(&state::StateSnapshot {
            message_count: 1,
            ..Default::default()
        });
    ...
    fn manual_invalidate_forces_a_rebuild() {
        ...
        {
            let state = rt.inner.read().unwrap();
            state.lua.load("kn9t.invalidate()").exec().unwrap();
        }
        rt.build_ui_outcome(100, 40);
