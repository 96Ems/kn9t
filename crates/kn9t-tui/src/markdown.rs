...
impl<'t> MarkdownRenderer<'t> {
    ...
    fn end_tag(&mut self, tag: TagEnd) {
        match tag {
            ...
            TagEnd::TableCell => {
                ...
                let cell_text: String = self.current_line.iter()
    ...
    fn wrap_and_push(&mut self, text: &str, style: Style) {
        for word in text.split_inclusive(char::is_whitespace) {
            ...
            if word_len > self.width {
                ...
                self.current_line.push(Span::styled(word.to_string(), style));
    ...
    fn current_style(&self) -> Style {
        ...
        // Link style.
        if self.in_link {
            style = style.fg(self.theme.primary).add_modifier(Modifier::UNDERLINED);
        }
