...
fn tool_execute_generates_unique_call_id_per_invocation() {
    ...
    let r1 = api.handle(
        ...
        Some(&sess.0),
        "tool_execute",
        &json!({"session": sess.0, "name":"bash","args":{"cmd":"echo 1"}}),
    );
    ...
    let r2 = api.handle(
        ...
        Some(&sess.0),
        "tool_execute",
        &json!({"session": sess.0, "name":"bash","args":{"cmd":"echo 2"}}),
    );
    ...
    assert_ne!(
        ids[0], ids[1],
        "CallIds must be unique per invocation, got {:?} and {:?}",
        ids[0], ids[1]
    );
    assert!(
        ids[0].starts_with("plugin-bash-"),
        "call id should be plugin-bash-{{counter}}, got {}",
        ids[0]
    );
    assert!(
        ids[1].starts_with("plugin-bash-"),
        "call id should be plugin-bash-{{counter}}, got {}",
        ids[1]
    );
}
...
fn live_tool_calls_not_overwritten_by_repeated_tool_execute() {
    ...
    api.handle(
        ...
        Some(&sess.0),
        "tool_execute",
        &json!({"session": sess.0, "name":"bash","args":{}}),
    )
    .unwrap();
    api.handle(
        "plug",
    ...
    for id in &ids {
        let _ = state
            .store
            .begin_live_tool_call(&sess, &CallId(id.clone()), "bash");
    }
    let _ = state
        .store
        .append_live_tool_progress(&sess, &CallId(ids[0].clone()), "first");
    let _ = state
        .store
        .append_live_tool_progress(&sess, &CallId(ids[1].clone()), "second");

    let rows: Vec<String> = state
        .store
        .query_strings(
            "SELECT call_id FROM live_tool_calls WHERE session_id=?1 ORDER BY call_id",
            &[&sess.0],
        )
        .unwrap_or_default();
    ...
    assert_eq!(
        rows.len(),
        2,
        "live_tool_calls should have 2 rows after fix, got {:?}",
        rows
    );
}
