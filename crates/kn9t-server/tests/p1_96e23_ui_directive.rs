...
fn plugin_can_push_structured_directive_scoped_to_one_session() {
    ...
    let res = api.handle("my-plugin", Some(&sess_a.0), "ui_directive", &json!({
        "session": sess_a.0,
        "target": "sidebar",
        "op": "show",
    ...
    let ev_a = sub_a.recv_timeout(std::time::Duration::from_secs(2)).expect("sess_a should receive UiDirective");
    match ev_a {
        kn9t_core::Event::UiDirective { plugin, target, op, payload } => {
            assert_eq!(plugin, "my-plugin");
            assert_eq!(target, "sidebar");
            assert_eq!(op, "show");
            assert_eq!(payload, json!({"panel":"debug","count":42}));
        }
        other => panic!("expected UiDirective on A, got {}", serde_json::to_string(&other).unwrap_or_default()),
    }
    // sess_b must NOT receive anything (no broadcast)
    assert!(sub_b.recv_timeout(std::time::Duration::from_millis(200)).is_none(), "sess_b must not receive directive for sess_a (no broadcast fallback)");

    // Also ui_push alias should work (ticket says ui_push)
    let res2 = api.handle("my-plugin", Some(&sess_b.0), "ui_push", &json!({
        "session": sess_b.0,
        "target": "main",
        "op": "update",
        "payload": {"text":"hello"}
    ...
    let ev_b = sub_b.recv_timeout(std::time::Duration::from_secs(2)).expect("sess_b should receive via ui_push alias");
    assert!(matches!(ev_b, kn9t_core::Event::UiDirective { .. }), "expected UiDirective via alias, got {}", serde_json::to_string(&ev_b).unwrap_or_default());
}
...
fn ui_directive_validation_rejects_missing_target_or_op() {
    ...
    let r1 = api.handle("p", Some(&sess.0), "ui_directive", &json!({"session": sess.0, "op":"show","payload":{}}));
    assert!(r1.is_err(), "missing target must be rejected");
    let r2 = api.handle("p", Some(&sess.0), "ui_directive", &json!({"session": sess.0, "target":"x","payload":{}}));
    assert!(r2.is_err(), "missing op must be rejected");
    let r3 = api.handle("p", Some(&sess.0), "ui_directive", &json!({"session": sess.0, "target":"","op":"show"}));
...
fn ui_directive_payload_is_opaque_structured_and_plugin_notification_still_works() {
    ...
    let res = api.handle("p", Some(&sess.0), "ui_directive", &json!({"session":sess.0,"target":"page","op":"render","payload": complex.clone()}));
    assert!(res.is_ok());
    ...
    match ev {
        ...
        other => panic!("expected UiDirective, got {}", serde_json::to_string(&other).unwrap_or_default()),
    }
    ...
    let sink = SessionSink::with_store(state.buses.bus_for(&sess.0), state.store.clone(), sess.clone());
    sink.emit(LiveEvent::PluginNotification { payload: json!({"plugin":"p","message":"hello free text"}) });
    let ev2 = sub2.recv_timeout(std::time::Duration::from_secs(2)).unwrap();
    match ev2 {
        kn9t_core::Event::PluginNotification { payload } => {
            assert_eq!(payload.get("message").and_then(|v| v.as_str()), Some("hello free text"));
        }
        other => panic!("expected PluginNotification, got {}", serde_json::to_string(&other).unwrap_or_default()),
    }
}
