...
fn pending_interaction_blocks_and_resolves_via_host_api_and_route() {
    ...
    let handle = std::thread::spawn(move || {
        ...
        let res = api.handle(
            ...
            Some(&sess),
            "interaction_request",
            &json!({"payload": {"question":"choose","choices":["a","b"]}}),
        );
        res.unwrap()
    });
    ...
    let ev = sub
        ...
        .expect("InteractionRequest SSE not emitted");
    let (id, plugin, payload) = match ev {
        kn9t_core::Event::InteractionRequest {
            id,
            plugin,
            payload,
        } => (id, plugin, payload),
    ...
    assert_eq!(
        resp2.status, 400,
        "reused id must be rejected after consumption"
    );
}
...
fn interaction_payload_is_opaque_not_interpreted_by_host() {
    ...
    let ev = sub
        .recv_timeout(std::time::Duration::from_secs(2))
        .expect("InteractionRequest not emitted");
    if let kn9t_core::Event::InteractionRequest { id, .. } = ev {
        let req = api::UiRespondReq {
            id,
            payload: json!({"age": 42, "extra": [1,2,3]}),
        };
        let resp = kn9t_server::routes::interaction::respond(&state, req);
