...
fn est_tokens(content: &[Content]) -> i64 {
    let total: usize = content.iter().map(|c| match c {
        Content::Text { text } => text.len(),
        Content::ToolCall { name, args_json, .. } => name.len() + args_json.len(),
        Content::ToolResult { content, .. } => content.iter().map(|x| match x {
            Content::Text { text } => text.len(),
            _ => 20,
        }).sum(),
...
pub fn project(session_id: &str, ts: i64, event: &Event) -> Vec<Row> {
    match event {
        ...
        Event::Compacted { seq, replaced, summary } => {
...
pub fn write_rows(conn: &Connection, rows: Vec<Row>) -> Result<(), StoreErr> {
    for row in rows {
        match row {
            Row::Message { session_id, seq, role, content_json, est_tokens, silent } => {
                conn.execute(
