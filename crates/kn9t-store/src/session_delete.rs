...
impl SqliteStore {
    ...
    pub fn delete_session(&self, session: &SessionId) -> Result<(), StoreErr> {
        ...
        let content_jsons: Vec<String> = {
            ...
            let mut out = Vec::new();
            while let Some(r) = rows.next().map_err(|e| StoreErr(format!("msg content row: {e}")))? {
        ...
        for table in &["events", "messages", "usage", "live_messages", "live_tool_calls"] {
            conn.execute(
                ...
                params![sid],
            ).map_err(|e| { let _ = conn.execute_batch("ROLLBACK"); StoreErr(format!("delete {table}: {e}")) })?;
        }
        // plugin_kv: scope = session_id for session-scoped entries.
        conn.execute(
            "DELETE FROM plugin_kv WHERE scope=?1",
            params![sid],
        ).map_err(|e| { let _ = conn.execute_batch("ROLLBACK"); StoreErr(format!("delete plugin_kv scope: {e}")) })?;
        conn.execute("DELETE FROM sessions WHERE id=?1", params![sid])
            .map_err(|e| { let _ = conn.execute_batch("ROLLBACK"); StoreErr(format!("delete session: {e}")) })?;
