...
use kn9t_plugin::{codec::{write_plugin_msg, PluginMsg}, PluginDeclaration, PluginHost, NoOpPluginKv};
use serde_json::json;
...
fn make_pipes() -> (Box<dyn Read + Send>, Box<dyn Write + Send>, Box<dyn Read + Send>, Box<dyn Write + Send>) {
    let (h_read, p_write) = channel_pipe();
    let (p_read, h_write) = channel_pipe();
    (h_read, h_write, p_read, p_write)
}
struct ChannelReader { rx: std::sync::mpsc::Receiver<Vec<u8>>, buf: Vec<u8>, pos: usize }
impl Read for ChannelReader {
    fn read(&mut self, out: &mut [u8]) -> std::io::Result<usize> {
        if self.pos >= self.buf.len() {
            match self.rx.recv() { Ok(d) => { self.buf=d; self.pos=0; }, Err(_) => return Ok(0) }
        }
...
impl Write for ChannelWriter {
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        self.tx.send(buf.to_vec()).map(|_| buf.len()).map_err(|_| std::io::Error::new(std::io::ErrorKind::BrokenPipe,"closed"))
    }
    fn flush(&mut self) -> std::io::Result<()> { Ok(()) }
}

struct RecordingSink { events: Mutex<Vec<LiveEvent>> }
impl EventSink for RecordingSink {
    fn emit(&self, e: LiveEvent) { self.events.lock().unwrap().push(e); }
}
fn decl(name: &str) -> PluginDeclaration {
    PluginDeclaration { name: name.to_string(), capabilities: vec![], hooks: vec![], tools: vec![], subscribed_events: vec![], provider: None }
}
...
fn broadcast_without_session_id_must_not_leak_to_master() {
    ...
    let master_sink = Arc::new(RecordingSink { events: Mutex::new(vec![]) });
    let child_sink = Arc::new(RecordingSink { events: Mutex::new(vec![]) });
    ...
    let ev = PluginMsg::Event { event: json!({"plugin":"leaky","kind":"agents.md","text":"hello"}) };
    write_plugin_msg(&mut plugin_writer, &ev).unwrap();
    // Also send a second event with unknown session_id to ensure no broadcast
    let ev2 = PluginMsg::Event { event: json!({"plugin":"leaky","kind":"x","session_id":"unknown-999","text":"nope"}) };
    ...
    assert_eq!(master_events, 0, "master must NOT receive untagged plugin event (broadcast leak): got {master_events}");
...
fn routed_event_with_session_id_goes_only_to_target() {
    ...
    let _keep_reader = p_read;
    let mut pw = p_write;
    let ev = PluginMsg::Event { event: json!({"plugin":"leaky2","kind":"y","session_id":"master-002","text":"for master"}) };
...
fn session_scope_restores_parent_after_child_turn() {
    ...
    drop(scope);
    assert_eq!(host.session_id().as_deref(), Some("parent-001"), "SessionScope must restore parent session id");
}
