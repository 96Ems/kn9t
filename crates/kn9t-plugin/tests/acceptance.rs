...
use kn9t_core::{
    Content, HookName, HookVeto, Message, ModelRef, MsgId, Role, StopReason, Tokens, Usage,
};
...
fn spawn_plugin_multi(
    ...
    std::thread::spawn(move || {
        ...
        for reply_body in replies {
            ...
            if write_plugin_msg(&mut writer, &reply).is_err() {
                break;
            }
        }
    });
...
mod plug {
    ...
    fn handshake() {
        ...
        let hello = HostMsg::Hello {
            proto: 1,
            kn9t: "0.1.0".to_string(),
        };
        write_host_msg(&mut host_writer, &hello).expect("host: write hello");
        let reply =
            kn9t_plugin::codec::read_plugin_msg(&mut host_reader).expect("host: read plugin hello");
        match reply {
            PluginMsg::Hello {
                name,
                hooks,
    ...
    fn spawn_real() {
        ...
        if !bin_path.exists() {
            eprintln!(
                "skip plug::spawn_real — binary not found at {}",
                bin_path.display()
            );
        ...
        let host =
            PluginHost::spawn(&bin_path, &[], Arc::new(NoOpPluginKv)).expect("spawn must succeed");
    ...
    fn hook_surface() {
        // before_tool_call → allow
        {
            ...
            let host = PluginHost::from_io(
                h_read,
                h_write,
                decl("p", vec![HookName::BeforeToolCall]),
        ...
        // after_tool_call → keep
        {
            ...
            let host = PluginHost::from_io(
                h_read,
                h_write,
                decl("p", vec![HookName::AfterToolCall]),
                Arc::new(NoOpPluginKv),
            );
        ...
        // before_request → keep
        {
            ...
            let host = PluginHost::from_io(
                h_read,
                h_write,
                decl("p", vec![HookName::BeforeRequest]),
                Arc::new(NoOpPluginKv),
            );
        ...
        // should_stop_after_turn → continue
        {
            ...
            let host = PluginHost::from_io(
                h_read,
                h_write,
                decl("p", vec![HookName::ShouldStopAfterTurn]),
                Arc::new(NoOpPluginKv),
            );
        ...
        // prepare_next_turn → keep
        {
            ...
            let host = PluginHost::from_io(
                h_read,
                h_write,
                decl("p", vec![HookName::PrepareNextTurn]),
                Arc::new(NoOpPluginKv),
            );
        ...
        // get_steering → empty
        {
            ...
            let host = PluginHost::from_io(
                h_read,
                h_write,
                decl("p", vec![HookName::GetSteering]),
                Arc::new(NoOpPluginKv),
            );
        ...
        // get_followup → empty
        {
            ...
            let host = PluginHost::from_io(
                h_read,
                h_write,
                decl("p", vec![HookName::GetFollowup]),
                Arc::new(NoOpPluginKv),
            );
            ...
            let host_a = Arc::new(
                PluginHost::spawn(
                    ...
                    &[
                        ...
                        (
                            "TEST_PLUGIN_REPLY",
                            r#"{"action":"replace","content":[{"type":"text","text":"from_a"}]}"#,
                        ),
                    ],
                    Arc::new(NoOpPluginKv),
                )
                .expect("spawn plugin-a"),
            );
            let host_b = Arc::new(
                PluginHost::spawn(
                    &bin,
    ...
    fn timeout() {
        ...
        assert_eq!(
            default_timeout(HookName::BeforeToolCall),
            Duration::from_millis(30_000)
        );
        assert_eq!(
            default_timeout(HookName::AfterToolCall),
            Duration::from_millis(2_000)
        );
        assert_eq!(
            default_timeout(HookName::BeforeRequest),
            Duration::from_millis(2_000)
        );
        assert_eq!(
            default_timeout(HookName::ShouldStopAfterTurn),
            Duration::from_millis(1_000)
        );
        assert_eq!(
            default_timeout(HookName::PrepareNextTurn),
            Duration::from_millis(1_000)
        );
        assert_eq!(
            default_timeout(HookName::GetSteering),
            Duration::from_millis(500)
        );
        assert_eq!(
            default_timeout(HookName::GetFollowup),
            Duration::from_millis(500)
        );
        assert_eq!(
            default_timeout(HookName::GetApiKey),
        ...
        if !bin.exists() {
            ...
            let host = PluginHost::from_io(
                h_read,
                h_write,
                decl("slow", vec![HookName::GetSteering]),
                Arc::new(NoOpPluginKv),
            );
    ...
    fn plug_concurrent_calls_no_block() {
        ...
        let t1 = std::thread::spawn(move || h1.get_steering());
        let t2 = std::thread::spawn(move || h2.get_steering());
    ...
    fn p1_96e5_session_context_isolation() {
        ...
        let host = PluginHost::from_io(
            h_read,
            h_write,
            decl("test", vec![HookName::BeforeToolCall]),
            Arc::new(NoOpPluginKv),
        );
        ...
        assert_eq!(
            seen.len(),
            2,
            "plugin should have seen 2 hook calls, got {:?}",
            seen
        );
        // Before fix: both will be "session-B" (last writer wins) -> fails
        // After fix: one "session-A", one "session-B" (in any order) -> passes
        assert!(
            seen.contains(&"session-A".to_string()),
            "should have seen session-A, got {:?}",
            seen
        );
        assert!(
            seen.contains(&"session-B".to_string()),
            "should have seen session-B, got {:?}",
            seen
        );
        // Also ensure they are distinct
        assert_ne!(
            seen[0], seen[1],
            "concurrent sessions must not share context, both saw {:?}",
            seen[0]
        );
    }
    ...
    fn p1_96e5_bus_isolation() {
        ...
        let a_failed = a_events
            .iter()
            .filter(|e| matches!(e, Event::HookFailed { .. }))
            .count();
        let b_failed = b_events
            .iter()
            .filter(|e| matches!(e, Event::HookFailed { .. }))
            .count();
        ...
        assert_eq!(
            a_failed, 1,
            "bus A should have exactly 1 HookFailed, got {}",
            a_failed
        );
        assert_eq!(
            b_failed, 1,
            "bus B should have exactly 1 HookFailed, got {}",
            b_failed
        );
    }
    ...
    fn p1_96e9_event_backlog_does_not_block_rpc() {
        ...
        // HookFailed would be emitted on timeout; check immediately (HookFailed path does not go through event channel)
        {
            ...
            let hook_failed = evs
                .iter()
                .filter(|e| matches!(e, LiveEvent::HookFailed { .. }))
                .count();
            assert_eq!(
                hook_failed, 0,
        ...
        let notif = evs2
            .iter()
            .filter(|e| matches!(e, LiveEvent::PluginNotification { .. }))
            .count();
        assert!(
            notif > 0,
            "at least some notifications should arrive, got {notif}"
        );
        assert!(notif <= 200, "cannot exceed sent");
        // And ensure RPC still completed (no late HookFailed appeared)
        let hook_failed2 = evs2
            .iter()
            .filter(|e| matches!(e, LiveEvent::HookFailed { .. }))
            .count();
    ...
    fn p1_96e9_transient_events_may_be_dropped_under_pressure() {
        ...
        assert!(
            elapsed < Duration::from_millis(450),
            "RPC must complete even under event flood, took {:?}",
            elapsed
        );
    ...
    fn p1_96e10_protocol_corruption_marks_unhealthy() {
        ...
        assert!(
            elapsed1 < Duration::from_millis(600),
            "first call should not hang, took {:?}",
            elapsed1
        );
        ...
        assert!(
            !host.is_healthy(),
            "host should be unhealthy after protocol corruption, but is_healthy==true"
        );
        ...
        assert!(
            ...
            "poison reason should mention protocol violation/parse, got {:?}",
            reason
        );
        ...
        assert!(
            ...
            "new call after poison should fail fast (<100ms), took {:?}",
            elapsed2
        );
    }
    ...
    fn p1_96e10_new_calls_fail_deterministically_after_corruption() {
        ...
        for _ in 0..3 {
            ...
            assert!(r.is_empty());
            assert!(
                e < Duration::from_millis(100),
                "subsequent poisoned call should fail fast, took {:?}",
                e
            );
    ...
    fn p1_96e10_restarted_host_has_clean_stream() {
        ...
        // Second host with fresh pipes must be healthy and able to serve
        {
            ...
            assert!(
                host.is_healthy(),
                "fresh host should stay healthy after clean call"
            );
        }
    }
    ...
    fn p1_96e17_host_api_request_roundtrip() {
        ...
        let script = std::thread::spawn(move || {
            ...
            assert_eq!(
                result["messages"][0]["seq"], 5,
                "handler output passes through"
            );
        });
    ...
    fn p1_96e17_host_api_request_error_reply() {
        ...
        assert!(
            host.is_healthy(),
            "an op error must not poison the connection"
        );
    }
    ...
    fn p1_96e17_remote_compactor_roundtrip() {
        ...
        assert_eq!(
            txt.as_deref(),
            Some("resume"),
            "summary message parsed from wire"
        );
    ...
    fn p1_96e17_remote_compactor_error_reply() {
        ...
        let err = rc
            ...
            .map_err(|e| e.clone())
            .err()
            .expect("error reply must surface as Err");
        assert!(
            err.contains("blew up"),
            "plugin error message propagates, got {err}"
        );
    }
} // mod plug
