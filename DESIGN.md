...
flowchart LR
    ...
    S8b["8b. internal-plugins/kn9t-tools<br/>bash+read+edit as subprocess plugin"]
    S9["9. kn9t-custom-provider (external plugin) + anthropic"]
...
| Q25 | tools (`bash`/`read`/`edit`) ship as a subprocess plugin (`kn9t-tools` binary); `kn9t-tools` crate becomes integration harness only | keep tools in-process; in-process "plugin" adapter | subprocess validates the full plugin code path; enables hot-reload; IPC overhead (~1ms) negligible vs tool execution time |
...
| Q31 | kn9t-custom-provider + Anthropic ship as subprocess provider plugins (`kn9t-custom-provider`, `kn9t-anthropic`) using `kn9t-plugin-sdk`; `RemoteProvider` in `kn9t-plugin` adapts the stream into the `Provider` trait. **Refined 2026-08-28:** `kn9t-custom-provider` is an *external* standalone crate in `plugins/` (outside the workspace); `kn9t-anthropic` stays bundled, keeping both distribution paths exercised | build as `kn9t-provider-custom` / `kn9t-provider-anthropic` workspace crates depending on `kn9t-provider-core`; or keep both plugins as workspace members | plugin route: no workspace dep bloat; providers become hot-reloadable; full provider-plugin code path validated in production; making the custom provider external enforces the SDK-only boundary structurally rather than by review (workspace membership shares a lock file and target dir and cannot prevent an in-tree dep creeping in); accepted cost: ~1ms IPC overhead per stream start, separate build step, and `binary` must be an absolute path |
